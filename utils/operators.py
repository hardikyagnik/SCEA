...
def evaluate(offsprings: dict, rep_pro: List[dict], platform:Platform):
    ...
    if rep_pro:
        ...
        indices = [list(range(len(offsprings['children'])))] + [list(range(len(d['representatives']))) for d in rep_pro]
    else:
        combi_per_ind = 1
        indices = [list(range(len(offsprings['children'])))] 

    combinations = list(it.product(*indices))
    N = len(offsprings['children'])*combi_per_ind

    for k in range(0, N ,combi_per_ind):
        idx, *ris = combinations[k]
        schedule = offsprings['children'][idx]
        duration_cost = get_duration_cost(schedule, offsprings['project'])
        fr_cost = get_failure_ratio_cost(schedule, platform)
        simis = []
        for j in range(k, k+combi_per_ind):
            idx, *ris = combinations[j]
            schedule_pair =  [schedule]
            projects = [offsprings['project']]
...
def get_toolbox(platform: Platform, args) -> base.Toolbox:
    ...
    toolbox.register("evaluate", evaluate, platform=platform)
